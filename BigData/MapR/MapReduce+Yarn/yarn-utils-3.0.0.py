...
https://github.com/mahadevkonar/ambari-yarn-utils/tree/master/yarn-utils
...
def getReservedStackMemory(memory):
  mem = str(memory)
  if (mem in reservedStack):
    return reservedStack[mem]
  if (memory <= 4):
    ret = 1
  elif (memory >= 512):
    ret = 64
...
def getReservedHBaseMem(memory):
  mem = str(memory)
  if (mem in reservedHBase):
    return reservedHBase[mem]
  if (memory <= 4):
    ret = 1
  elif (memory >= 512):
    ret = 64
